...
class CsvFormatTest extends TestCase {
  ...
  public function quotingAccessors() {
    $format= new CsvFormat();
    $format->setQuoting(\text\csv\Quoting::$ALWAYS);
    $this->assertEquals(\text\csv\Quoting::$ALWAYS, $format->getQuoting());
  }
  ...
  public function withQuotingAccessor() {
    $format= new CsvFormat();
    $this->assertEquals($format, $format->withQuoting(\text\csv\Quoting::$ALWAYS));
    $this->assertEquals(\text\csv\Quoting::$ALWAYS, $format->getQuoting());
  }
  ...
  public function defaultFormatUnchangeableBySetQuoting() {
    CsvFormat::$DEFAULT->setQuoting(\text\csv\Quoting::$ALWAYS);
  }
  ...
  public function withQuotingClonesDefaults() {
    $format= CsvFormat::$DEFAULT->withQuoting(\text\csv\Quoting::$ALWAYS);
    $this->assertFalse($format === CsvFormat::$DEFAULT);
  }
}
