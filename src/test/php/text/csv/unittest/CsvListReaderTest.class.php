...
class CsvListReaderTest extends TestCase {
  ...
  public function multiLine() {
    $in= $this->newReader(
      ...
      create(new \text\csv\CsvFormat())->withQuote("'")
    );
    $this->assertEquals(
  ...
  public function tabDelimiter() {
    $in= $this->newReader("A\tB\tC", \text\csv\CsvFormat::$TABS);
  ...
  public function spaceDelimiter() {
    $in= $this->newReader('A B C', create(new \text\csv\CsvFormat())->withDelimiter(' '));
    $this->assertEquals(array('A', 'B', 'C'), $in->read());
  }
